/**
 Date: 09-28-2021
 Description:
 i've decided to use lines of varying thicknesses rather than cylinders.
 current problem: when rendering as is, all hairs(?) are clumped together in one
 corner & the terrain is missing. wasd doesn't seem to work
 
 terrain problem fixed: it was because i put it inside the beginShape code originally
 & that broke it
 
 currently all hairs are stuck to 1 corner & moving arbitrarily. i have to find a
 way to set the hairs permanently at startup instead of drawing over & over in draw
